...
//                    })
    ...
    func getRandomCardColor(){
        let listOfCardColors: [Color] = [.white, .white, .black, .red, .green]
    ...
    func yesWasClicked(){
        ...
        if cardColor == Color.red{ score = 0 }
        
        if cardColor == Color.green{ score += 10 }
...
struct Hard_Previews: PreviewProvider {
    static var previews: some View {
        Hard(topColor: "black", bottomColor: "red", randomUIColor: .green, cardColor: .white)
    }
}
